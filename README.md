<h1 align="center">Hi 👋, I'm Ankush Singh</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=600&lines=Computer+Science+Undergraduate;Full+Stack+Developer+(MERN);AI+%26+Computer+Vision+Enthusiast;Building+Real-World+Systems" />
</p>

---

## 👨‍💻 About Me

🎓 2nd-year **B.Tech Computer Science student**  
💡 Passionate about **AI, Machine Learning & Computer Vision**  
🛠️ Strong foundation in **Full-Stack (MERN)** development  
🧠 Curious about **how systems work internally**  
📈 Focused on **clean, scalable, and efficient code**

I enjoy building **impact-driven projects**, learning concepts **in depth**,  
and solving problems that push my technical thinking.

---

## 🛠️ Tech Stack

### 💻 Programming Languages
<p>
  <img src="https://skillicons.dev/icons?i=c,cpp,java,python,javascript" />
</p>

### 🌐 Web & Backend
<p>
  <img src="https://skillicons.dev/icons?i=html,css,tailwind,react,nodejs,express,mongodb" />
</p>

### 🤖 AI / ML
<p>
  <img src="https://skillicons.dev/icons?i=python,tensorflow,opencv" />
</p>

### 🧰 Tools & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,linux,postman" />
</p>

---

## 🚀 Featured Projects

### 🔹 Chakshu_Rakshak  
**Women Safety Intelligence Platform**
- Safety heatmap using crime & user data  
- Safe-route navigation instead of shortest path  
- Emergency alerts & real-time assistance  

### 🔹 Fog-Adaptive Object Detection  
**YOLO + Saliency Fusion**
- Fog severity estimation  
- Adaptive confidence thresholding  
- Robust detection in low-visibility environments  

### 🔹 MERN Authentication System  
- Secure login/signup with JWT  
- Clean backend architecture  
- Scalable and production-ready design  

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight" height="160"/>
</p>

---

## 📫 Connect With Me

- 💼 LinkedIn: <a href="https://www.linkedin.com/in/ankush-singh-7v7/"> https://www.linkedin.com/in/ankush-singh-7v7</a>
- 📧 Email: <a href="ankushsinghv77@gmail.com"> ankushsinghv77@gmailcom </a>
- 🌐 Portfolio: 

---

### ⚡ Philosophy

> *Depth over speed.  
> Understanding over memorization.  
> Code that lasts matters more than code that passes.*


<!--
**Ankushsv/Ankushsv** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
